...
\begin{abstract}
	...
	The present paper reports a case study of the implementation of an information system for data analysis, aiming to demonstrate the analysis of multidimensional data cubes on the client side.
...
\begin{resumo} 
	...
	O presente trabalho relata um estudo de caso referente a implementação de um sistema de informação para análise de dados, tendo como objetivo demonstrar a realização de análises sobre cubos de dados multidimensionais no lado cliente.
...
\section{Referencial teórico}\label{referencial-teorico}
	...
	\subsection{OLAP}
		...
		Em um contexto mais analítico e interativo, voltado para tomada de decisão rápida sobre grandes volumes de dados, o cenário se estreita a análises sobre cubo de dados multidimensional.

		Nesse sentido, entra a tecnologia OLAP (On-Line Analytical Processing) que significa processamento analítico online.
		OLAP nada mais é do que uma tecnologia de banco de dados otimizado na forma de cubo de dados multidimensional, diferente de um banco de dados relacional baseado em linhas e colunas.
		Os dados são visualizados sob diversos ângulos chamados de dimensões, onde uma dimensão que sempre existe é o tempo.
	...
	\subsection{Conceitos de cubo de dados}
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.4\textwidth]{cubo-dados-multidimensional.jpg}
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.4\textwidth]{modelo-multidimensional-estrela.jpg}
	...
	\subsection{JSON}
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.3\textwidth]{json-formato.jpg}
		...
		Este formato de dados tem sido bastante utilizado no desenvolvimento de \hl{APIs (\emph{Application Programming Interface}) também aparecendo em serviços REST} (REpresentational State Transfer), em substituição ao formato XML (eXtensible Markup Language) que é mais verboso como se pode perceber no comparativo representado na figura \ref{fig:js-xml-comparativo}.
		
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.8\textwidth]{json-comparativo-xml.jpg}
	...
	\subsection{MVC}\label{mvc}
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.5\textwidth]{mvc.png}
	...
	\subsection{MVVM}
		...
		Um novo padrão de \emph{design} de \emph{software} é o MVVM.
		Significa Model-View-ViewModel e é uma variação dos padrões MVC e MVP vistos anteriormente.
		A novidade aqui é a camada ViewModel, que surge com a responsabilidade de disponibilizar para a camada View uma lógica de apresentação.
		Além disso, coordena interações entre as camadas Model e View sem ter nenhum conhecimento sobre a View.
		Outra vantagem é a sua testabilidade, uma vez que independe de Model e View.
...
\section{Status quo}
	...
	\subsubsection{HyperCube}\label{sec:hypercube}
		...
		HyperCube é uma biblioteca escrita em JavaScript que proporciona a criação de cubos de dados multidimensional, permite a aplicação de filtros e agregações, gerando dados estatísticos para fins de análise.
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.8\textwidth]{hypercube-mapa.jpg}
		...
		\begin{figure}[ht]
		\centering
		\includegraphics[width=.6\textwidth]{hypercube-cubo.jpg}
	...
	\subsection{Frameworks MVC client-side}

		Aplicações \emph{client-side} trazem a experiência de aplicações \emph{desktop} para aplicações \emph{web}.
		Atualmente existem diversos \emph{frameworks} MVC \emph{client-side}.
		A maioria deles visam melhorar a estrutura de aplicações \emph{web}, envolvendo recursos de MVC, rotas e mecanismos de \emph{templating}.
		Cada um tem suas próprias particularidades, diferenciando principalmente em relação a camada de apresentação (View).
		Em seguida, serão apresentados alguns dos populares \emph{frameworks} MVC \emph{client-side}.

	\subsubsection{Ember.js}

		Um dos \emph{frameworks} MVC \emph{client-side} populares para se criar aplicações de página única (SPA) é o Ember.js, já utilizado por empresas como Yahoo! e Groupon.
		O Ember.js segue o conceito de CoC (Convention over Configuration), onde o desenvolvedor configura a aplicação por meio de convenções, podendo focar no que realmente importa.

		Para a criação dos \emph{templates}, o Ember.js usa por padrão a biblioteca JavaScript Handlebars, mas permite o uso de outras formas de \emph{templating}.
		O fator desempenho tem sido um dos alvos do Ember.js.
		Mecanismos de cache e pré-compilação de \emph{templates}, ajudam a aplicação a carregar e rodar com boa performance.

	\subsubsection{Backbone.js}

		Criado em 2010, o Backbone.js é um \emph{framework} MVC \emph{client-side} muito leve, utilizado por serviços como Pinterest, AirBNB e Flixster.
		Possui código simples e é fortemente documentado.

		Entre as suas principais características, destaco a sua flexibilidade e facilidade de extensão, permitindo que o desenvolvedor crie seu próprio \emph{framework}, baseado no Backbone.js.
		Sua curva de aprendizado para usar o Backbone.js pode ser menor, caso o desenvolvedor já tenha experiências com o \emph{framework} Underscore.js e jQuery.

	\subsubsection{AngularJS}

		AngularJS é um \emph{framework} MVC \emph{client-side} criado em 2009 de bastante destaque sendo atualmente patrocinado pela empresa Google.
		Vem trazendo conceitos inovadores para os desenvolvedores de aplicações \emph{web}, por exemplo o conceito \emph{Two-way data binding} que mantém informações sincronizadas entre as camadas Model e View.
		Uma vez que um dado é alterado na página, o mesmo é refletido no modelo.
		Da mesma forma, uma vez que o dado é modificado no modelo, se reflate automaticamente na página.
		Em seu site oficial, é mencionado como \emph{"Superheroic JavaScript MVW Framework"}, devido ao seu poder e abrangência dos paradigmas MVC e MVVM.

		O AngularJS é construído sobre a ideologia de que a programação declarativa deve ser usada para construção de interfaces e componentes, enquanto que a programação imperativa é excelente para escrever as regras de negócio \cite{col-front-end}.
		Uma das suas características é sua arquitetura de \emph{building blocks}, que mantém categorias distintas de código como diretivas e filtros, além dos tradicionais \emph{controllers} e \emph{views}.
...
\section{Descrição da solução}\label{descricao-da-solucao}
	...
	\subsection{Statz}\label{statz}
		...
		Tem como característica possibilitar esta análise sobre cubos de dados multidimensionais (OLAP), portanto através de filtros e segmentações o Statz apresenta os dados para análise através de \emph{dashboards}.
		...
		\subsubsection{Universo de dados}\label{statz-universo-de-dados}
			...
			Para servir de exemplo da análise de dados usando sistema Statz, foi escolhida uma base de dados de histórico de copas do mundo de futebol até 2010, fornecida pelo site da FIFA (Fédération Internationale de Football Association) como mostra a figura \ref{fig:site-fifa}.
			...
			\begin{figure}[ht]
			\centering
			\includegraphics[width=1\textwidth]{fifa-2010.png}
		...
		\subsubsection{Arquitetura}\label{statz-arquitetura}
			...
			\begin{figure}[ht]
			\centering
			\includegraphics[width=1\textwidth]{json-participacoes-copas.png}
...
Para importar os dados e trabalhar as análises, busca um objeto JavaScript no formato JSON para a geração do cubo de dados multidimensional.
...
Com os dados no lado cliente, o Statz já pode montar o cubo de dados multidimensional e manipular os dados. 
...
\end{comment}
		...
		\subsubsection{Análise histórica}
			...
			Se quisermos filtrar por mais de um ano, País ou continente, o resultado será um cubo de dados vazio.
			...
			\begin{figure}[ht]
			\centering
			\includegraphics[width=.8\textwidth]{statz-historico.png}
		...
		\subsubsection{Análise de ranking}
			...
			\begin{figure}[ht]
			\centering
			\includegraphics[width=.8\textwidth]{statz-ranking.png}
		...
		\subsubsection{Análise específica de um País}
			...
			\begin{figure}[ht]
			\centering
			\includegraphics[width=.8\textwidth]{statz-pais.png}
...
\section{Próximos passos}
	...
	\hl{Lembrando que a análise é sobre dado histórico (BI/Big Data), portando não necessita atualização em tempo real.}

	Já pensando em carregar os dados em tempo de execução, \hl{o sistema poderia consumir uma API para trazer os dados no formato esperado pelo JavaScript e na estrutura esperada pelo HyperCube. Como a aplicação trabalha no formato JSON e o Hypercube precisa de uma estrutura de dados específica, uma excelente abordagem é utilizar bancos de dados orientados a documentos (NoSQL).}
	
	\hl{parágrafo de bancos NoSQL removido}
	\begin{comment}
	...
	\hl{parágrafo de Websocket removido, parte dele incluída acima junto a carga em tempo de execução}
	\begin{comment}
	Outra abordagem interessante seria manter o dado em tempo real via WebSocket, onde a aplicação se inscreveria em uma fila e receberia eventos que poderiam ser utilizados para atualizar os dashboards das páginas de análise.
